...
This project is a mobile web app for recording date-based events in chronological order.

## Core Principles
- **Efficiency & Clarity**: The app is designed for a single user with a focus on speed of input and browsing.
- **Utility over Aesthetics**: It just needs to be functional and readable.
- **Mobile-First**: Optimized for mobile browser usage.
- **Ergonomics**: Interactivity essential for day-to-day use (like input) should be reachable with a thumb (placed at the bottom). Non-essential interactivity (like settings or export) belongs at the top.

## Technical Stack
- **Vanilla JS & CSS**: No external libraries or frameworks.
- **No Dependencies**: Avoid outside dependencies; the app should be self-contained.
...
- **Top Controls**: Includes features like theme toggling and data export.
